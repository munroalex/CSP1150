...
#while loop while continue until user quits the script
while quit == False: 
    #prompt user for input and convert to lowercase   
    user_input = str(input('Enter a roll ("h" for help, "x" to quit):')).lower()
    ...
    #check if user has quited script and display final statistics before quiting
    elif user_input == 'x':              
        quit = True
    ...
    #check if the user input contains a letter 'd' and split into a list on the letter 'd', if no letter 'd' detected prompt the user it was an invalid roll and restart loop
        ...
        if len(dice_roll) == 2:
            #attempt to convert the first value (quantity) to an integer, if this fails prompt the user it was an invalid roll and restart loop
            try:
        ...
        for roll in range(quantity):
            ...
            if exploding_dice == True and individual_roll == sides:
                ...
                while exploding == True:
                    ...
                    #check to see if a exploding roll has failed to acheive a max roll and quit exploding loop
                    if kaboom_roll != sides:
                        explosion_number_list.append(single_dice_explosion)
                        exploding = False
                roll_list.append(sum(exploding_list))
            else:
        ...
        print(f'Average: {int(round((sum(roll_list)/len(roll_list)),0))}')
